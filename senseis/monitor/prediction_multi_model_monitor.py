...
def create_multi_volatility_prediction_gauges(app_name, exchanges, config_file):
  ...
  call_signs = set([get_pred_exchange_call_sign(exchange) for exchange in exchanges if is_multi_volatility_pred_exchange(exchange)])
...
def create_multi_return_prediction_gauges(app_name, exchanges, config_file):
  ...
  MULTI_RETURN_PREDICTION_GAUGES = {
    pid : {
      call_sign : {
        f"{pid}:{target_name}" : Gauge(f"{app_name}_{pid.replace('-','_')}_{target_name}",
                                       f"{pid} {target_name}",
...
def model_monitor(data, exchange_name):
  ...
  elif is_multi_return_pred_exchange(exchange_name):
    gs = get_return_prediction_gauges()
    for (key, val) in data.items():
      if key in gs[pid][call_sign]:
        gs[pid][call_sign][key].set(data[key])
